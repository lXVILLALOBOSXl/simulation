...
/*    private double meanSquared(){
    ...
    private double calculateIntersect(){
        ...
        for (int i = 0; i < n; i++){
            sum += (this.dataSet.getY()[i] - (this.beta[0] + (this.beta[1]*this.dataSet.getX()[i])));
        }
        return (-2/n) * sum;
    }
    ...
    private double calculateSlope(){
        ...
        for (int i = 0; i < n; i++){
            sum += (this.dataSet.getX()[i] * (this.dataSet.getY()[i] - (this.beta[0] + (this.beta[1]*this.dataSet.getX()[i]))));
        }
        return (-2/n) * sum;
    }
